...
public class BoardController {
	...
	private final Colour[] playerColours = { Colour.RED, Colour.GREEN, Colour.BLUE, Colour.YELLOW }; // Add player
																										// colours array
...
// Create panel configurations first as they are needed for setupUI and drawGameBoard
		...
		setupBoardUI(playerPanelConfig); // Passs the config to setup UI elements
...
// Initial drawing of board elements is now moved to the constructor after firedeck init
	...
	private Map<Integer, BoardView.PlayerPanelInfo> createPlayerPanelConfigurations(String humanPlayerName) {
		...
		List<Integer> logicalPlayerIndices = Arrays.asList(0, 1, 2, 3); // Represents logical player indices (RED=0,
																		// GREEN=1, BLUE=2, YELLOW=3)

		Collections.shuffle(logicalPlayerIndices, new Random()); // Shuffle which logical player color gets which panel
																	// position
...
// Find the logical player index that was randomly assigned to visual position 0 (BOTTOM)
		...
		int logicalPlayerIndexForBottomPanel = logicalPlayerIndices.indexOf(0); // Find the original index of the player
																				// who ended up at visual position 0
...
// Get the logical player index that was randomly assigned to this visual panel position
			...
			int logicalPlayerIdxAssignedToPanel = logicalPlayerIndices.get(visualPanelIndex); // This index corresponds
																								// to the color, name,
																								// icon, etc.
...
// --- Revised Shuffling Logic ---
		...
		List<Integer> panelPositionIndices = Arrays.asList(0, 1, 2, 3); // Represents visual panel indices (0=Bottom,
...
// Assuming for now that the humanPlayerName reliably matches one of the default names initially.
		...
		for (BoardView.PlayerPanelInfo info : panelConfigurations.values()) {
			...
			if (info.name.equals(humanPlayerName) && !info.name.startsWith("Mr.")) { // Simple check to see if the name
																						// was customized
